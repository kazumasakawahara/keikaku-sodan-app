...
### Phase 4: AI計画作成支援機能（現在利用可能）✅

#### 1. AI計画作成支援（Ollama ローカルLLM）
- **ローカルAI**による計画提案（インターネット不要・個人情報保護）
- 利用者情報の総合的分析
  - 基本情報（年齢、性別、障害支援区分）
  - 障害特性・興味の偏り
  - 現在の服薬情報
...
- 日本語対応モデルのダウンロード（llama3推奨）
- ollama Pythonパッケージ（自動インストール）
...
1. **Ollamaのインストール**
   - https://ollama.com からOllamaをダウンロードしてインストール
...
2. **日本語対応モデルのダウンロード**
   ...
   ollama pull llama3
   ```
   - 初回のみ約4.7GBのダウンロードが必要です
   - ダウンロードには5-10分程度かかります
...
1. 画面上部のナビゲーションから「計画一覧」→「新規計画作成」をクリック
2. 「利用者」のプルダウンメニューから対象の利用者を選択
3. 「AI提案を生成」ボタンが有効になるのでクリック
4. AIが20-30秒程度で計画案を生成します
   - 生成中は画面にローディング表示が出ます
5. 生成が完了すると、以下の項目が自動入力されます：
   - 現在の状況
   - 本人・家族の希望やニーズ
   - 総合的な援助方針
   - 長期目標
   - 短期目標
6. 生成された内容を確認・修正して「作成」ボタンをクリック

#### AIが参考にする情報

AI計画提案は、以下の情報を総合的に分析して生成されます：
- ✅ 利用者の基本情報（年齢、性別、障害支援区分）
- ✅ 障害特性・興味の偏り（登録されている場合）
- ✅ 現在服用中の薬の情報
- ✅ 最近の相談記録（直近5件）
- ✅ 前回の計画と評価（前回計画を指定した場合）

**💡 ポイント:**
- 利用者情報が充実しているほど、より具体的な提案が生成されます
- 相談記録を定期的に入力することで、よりその人に合った提案になります
- 生成された提案はあくまで「たたき台」です。必ず内容を確認・修正してください

#### プライバシー保護

- **すべての処理はパソコン内で完了**します
- インターネットに個人情報が送信されることはありません
- Ollamaはローカルで動作するため、安心して利用できます
